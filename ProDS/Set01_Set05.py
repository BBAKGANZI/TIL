...
q2.nlargest(2) # 최대값의 값과 인덱스명

q2.min() # 0.999497444941335, 최소값
q2.argmin() # 위치번호
q2.idxmin() # 최소값이 있는 인덱스명
q2.nsmallest(2) # 최소값의 값과 인덱스명
...
# =============================================================================
# 채널별 마케팅 예산: 수치로 표현되어 있어야함
# 매출금액: 수치(회귀분석), 범주(분류분석)
# 매출액: Y, 종석변수: X
# =============================================================================
...
# form2 = 'Sales~' +'+'.join(x_list)+'-1' # 상수항미포함
...
# bonf(p)로 귀무가설확인
ols1.outlier_test()
ols1.outlier_test()['unadj_p'] < 0.05
# 이상치 체크 
...
params1.sort_values(ascending=False) # 내림차순
...
q1 = pd.crosstab(index=[data2.Sex, data2.BP], 
                 ...
                 normalize=True) # True 대신에 숫자, 'all' 사용가능
...
q1_tab = pd.pivot_table(q1, index = 'TIME',
                        values = 'Value',
                        aggfunc = 'sum').reset_index() # reset_index() 인덱스였던 TIME을 data로
...
q2 = data4[data4.LOCATION.isin(['KOR', 'JPN'])] # isin 괄호안에 값만 필터링
...
# (3) 대응 t-test(H0: 차이가 없다, H1: 차이가 있다)
